...
# client = tradeapi.REST("API_KEY", "SECRET_KEY", "https://paper-api.alpaca.markets", 'v2')
...
for i in range(mva_period, len(closing_prices)):
    ...
    if p > (mva * 1.02) and last_qty > 0:  # when shorting >= 0:
        ...
        for s in tickers_data.iterrows():
            print(":::" + str(s[0]) + ":::")
        print('if ' + str(tickers_data.iterrows()) + ' is ' + str(date.datetime.now()))
        if tickers_data.iterrows().__next__()[0] is date.datetime.now():
            print('Sell')
    ...
    if p < (mva * 0.98) and last_qty <= 0:
        ...
        print('if ' + str(tickers_data.iterrows().__next__()[0]) + ' is ' + str(date.datetime.now()))
        if tickers_data.iterrows().__next__()[0] is date.datetime.now():
            # stop_price = (p * 0.95)
            print('Buy')
